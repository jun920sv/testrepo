...
public class ToDoController {
	...
	*/
	...
	public ModelAndView edit(
	        @PathVariable Integer id,ModelAndView model,RedirectAttributes attributes) {
	    Optional<ToDo> target = toDoService.findByIdToDo(id);
	    target.ifPresentOrElse(
	        t -> {
	            ToDoForm form = ToDoHelper.convertToDoForm(t);
	            model.addObject("toDoForm", form);
	            model.setViewName("ToDo/form");
	        },
	        () -> {
	            attributes.addFlashAttribute(
	                    "errorMessage","対象のデータがありません");
	            model.setViewName("redirect:/todos");
	        });
